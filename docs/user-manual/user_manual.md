...
点数制限が有効な間は、表示されない希少イベントがあり得るため、plot下部に表示範囲の件数と「gates and statistics use all events」という注意を出す。希少集団を目視確認する場合は上限を増やすか `0 (all events)` を選ぶ。

`Display max points` による点数制限、色、背景、凡例、フォント、軸ラベル、オーバーレイは**表示専用**である。ゲート membership、イベント数、frequency、統計、TSV/CSV 出力は全イベントを使う。サンプル切替時のprocessed displayは最近使用した表示結果を一時的にcacheするが、メモリ上限に達すると古い表示cacheを自動破棄して必要時に再計算する。このcache制御はrawイベントやauthoritativeな解析結果を変更しない。4 MiB以上のFCSを選択した場合は、読込をバックグラウンドで行うため、読込中もGUI操作を継続できます。短時間に別sampleへ切り替えた場合、古い読込結果は表示へ適用されず、最後に選択したsampleが優先されます。小さいFCSは従来どおり同期読込です。
...
|Credits...|著作権者、copyright year、連絡先、BSD 3-Clause License を表示する。完全な本文は配布物の `LICENSE` にある。|
...
長いMessageは列幅に依存せず、Messageセルへマウスを置くとtooltipで全文を確認できる。診断行を選択すると、表の下の読み取り専用詳細欄にも全文が表示され、コピーできる。ゲートやStatisticのIDが含まれる場合は、対応する名前・ゲート種別・metric・parameterも併記される。選択を解除した場合は詳細欄が空になる。
...
|Expression inputs|Expressionから安全なparserで自動検出した参照parameterを、画面上の短縮表示名と正式名で表示する。読み取り専用で、式が未完成・不正な間はその状態を表示する。|
...
- `Matrix Preview`: Matrix ID、Name、Source、Notes、Channels、全選択/全解除、
  ...
  X transform / Y transformでLinear、Log10、Asinhを選択できる。補償計算自体はraw値で行われ、
...
大規模出力の比較には、`python tools/benchmark_vector_scatter.py`を使用できます。既定では
1k、5k、20k、100k、1M点について、sparse/dense/overlap/mixed fixtureを選択して3 modeの
出力bytes、経過時間、peak RSS、SVGのuse/path数、PDFのForm/Image数をJSONへ保存します。
...
保存済み定義がある場合は、ダイアログの `Run Saved Queue` で定義を一覧の順番にまとめて実行できます。`Queue failure policy` は既定の `Fail fast`（最初の失敗で停止）と `Continue after failures`（失敗を記録して次へ進む）から選びます。各定義はoutput directory下の番号付きサブディレクトリへ出力され、全定義で同じキャンセルとruntime policyを共有します。thread/memory、density worker設定はGUIでは検証完了まで無効化されています。キュー開始時にSample Sheet、タイトル、plot presentationなど現在のGUI状態が未保存なら、既存projectへ先に保存してから実行します。保存に失敗またはキャンセルされた場合はキューを開始しません。進捗windowの `Cancel` は現在の定義の安全な境界で停止し、完了済み定義の出力とmanifestを保持します。
...
複数の保存済み定義をCLIで順番に実行する場合は、`--queue-export-id <id>` を繰り返します。project内の保存済み定義を宣言順ですべて実行する場合は `--queue-all` を使用できます。`--queue-all` はprojectを一度だけ読み込み、同じsnapshotを各定義へ渡します。
