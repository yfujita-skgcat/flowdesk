...
- A definition queue loads one immutable project snapshot at queue start and passes it to
  each definition. The queue also uses a bounded, fingerprint-scoped LRU for immutable raw
  FCS sample objects. Transformed layers, population masks, density fields, renderer caches,
...
- CLI-only `--queue-workers N` provides explicit definition-level bounded parallelism.
  ...
  of each definition's target/overlay source set plus raster/hybrid output canvas working
  set limits queue workers further; the maximum resolved estimate, budget, and limiting
  factor are written to queue provenance. If a
