...
- [ ] 再開: Canonical `PlotScene`のwriter共有までは完了しているが、current-view右クリックExportとBatch/CLIがscene/payloadを別々に構築しているため未完了。詳細は`docs/implementation/current-view-export-batch-parity.md`を正とする。
- [ ] Parity Increment 0: 現行Batch出力をcharacterization testで固定する。3-source overlay、Log10 tick、rectangle/polygon gate、長いaxis label、1〜3行title、96/300 DPIについて、source/title order、色、tick、gate points/style、plot rect、scene hash、point-plan hashを記録し、以後のincrementでBatch出力が変わっていないことを必須条件にする。
- [ ] Parity Increment 1: `flowdesk_core`にtyped `PreparedPlotRenderPayload`を追加し、semantic source order、back-to-front draw order、source ID keyed layers/colors、coordinate space、scene/options/provenanceを分離する。旧projectのdraw orderは既存Batch source orderへfallbackする。
- [ ] Parity Increment 2: Batchのtick生成、range解決、gate正規化/clipping、source style/title解決、layer正規化、writer dispatchをcore serviceへ移す。Increment 0のBatch sidecar/image/hashが完全一致しない限り先へ進まない。
- [ ] Parity Increment 3: GUIからactive sample、semantic title order、painter draw order、source ID keyed rendered arrays、parameter/transform IDs、coordinate space、live ViewBox、labels/ticks/layout measurements、Sample Sheet title/color、gate IDsを持つcurrent-view snapshotを作る。Qt側で科学的transform/gate/title precedenceを解決しない。
- [ ] Parity Increment 4: current-view snapshotをBatchと同じcore service/payload/dispatcherへ渡す。`_current_plot_export_metadata()`と`_export_current_plot_core()`からpositional layer zip、title/color/tick/gate再構築を削除する。
- [ ] Parity Increment 5: `render_batch_plot_qt()`のdual-mode scene再構築と無視される引数を削除し、全user-facing PNG/JPEG/SVG/PDFをtyped payload経路へ統一する。Qt screenshot/grabはtest専用とする。
- [ ] Parity Increment 6: 同一定義のGUI screenshot、右クリックExport、Batch Exportを比較し、title text/order/color、semantic/draw order、axis labels/bounds、Log10 ticks、gate geometry/style、view range、plot rect、96/300-DPI正規化画像が一致することを確認する。raw events、membership、count/frequency/statistics/revisionが不変であることも検証する。
...
- [ ] outputをderived parameter、Population、table、artifactとして検証してimportする。
  ...
  - [x] CLI-only `--queue-workers N`による定義単位bounded並列を追加する。各定義のinner backendを
    ...
    samplesへ保守的にfallbackする。PNG/JPEG/hybrid PDFでは解決済みcanvas working setも加算し、
    大きな出力設定による過大なqueue concurrencyを抑制する。
...
- [x] `tools/benchmark_batch_plot.py --project <bundle> --queue`を追加し、保存済み全definitionの
...
- [x] **2026-07-31 queue smoke**: `data/analysis.flowdesk --queue --max-workers 2`を実FCSで実行し、
  ...
  保存定義が1件のため実効workerは1であり、これはqueue並列化や実FCS I/O speedupの根拠ではない。
...
- [x] Increment 1: Sample Browserのcanonical manual order、drag-and-drop、`Ctrl+Up`/
...
- [x] Increment 2: Add Statisticのparameter selectorについて、metric policy、catalog
...
- [x] Increment 3: applicationを終了しない`File -> Close Project`を追加する。dirty save/
...
- [x] Increment 5: `docs/implementation/statistic-value-selection.md`を正として、Statisticsの
  ...
  transform、gate、Results、save/load、CLIの数値とIDを非退行testで固定した。guideの4 incrementを
  順に実装し、core/GUI/CLI parity testとユーザーマニュアル更新を完了した。
