...
- [ ] current-view右クリックExportとBatch/CLI exportのrenderer入力を共通core payloadへ統合する。active/manual/advanced overlayのsource style、タイトル色、Sample Sheet title、axis label、gate style、tickを一度だけ解決し、同じ表示definitionのPNG/SVG/PDF sidecarと画像を比較する。NaN表示イベントは共通入口で描画対象から除外する。詳細は`docs/implementation/plot-export-completion.md`の「Current-view and Batch parity repair」。
