...
- [x] Canonical `PlotScene`のwriter共有をcurrent-view右クリックExportにも適用し、Batch/CLIと同じcore preparation/writerへ接続した。タイトルの意味順とドットの描画順、live ViewBox/ticks/layout、gate geometry/styleを別フィールドで保持する。残るlegacy adapter整理はParity Increment 5として継続する。詳細は`docs/implementation/current-view-export-batch-parity.md`を正とする。
...
- [x] Parity Increment 3: GUIからactive sample、semantic title order、painter draw order、source ID keyed rendered arrays、parameter/transform IDs、coordinate space、live ViewBox、labels/ticks/layout measurements、Sample Sheet title/color、gate IDsを持つcurrent-view snapshotを作る。Qt側ではgateを分析せず、coreのnormalizationへ渡す。
...
- [x] Batch overlayのタイトル順をSamples listのsemantic orderへ統一する。manual overlayはタイトル・metadataを元の順序で保持し、ドットの前後関係だけ`source_draw_order`で反転する。3-source回帰テストでGUI相当のタイトル順と描画順を別々に検証する。
